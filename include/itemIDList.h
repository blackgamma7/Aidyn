
//item types stored in high byte of item ID
enum DB_TYPE{
	DB_EMPTY, //No item ID
    DB_MISC, //Misc./Key/crafting item
    DB_ENTITY, //Player characters/NPC's/Enemies
    DB_SPELL, //Magic
    DB_RITUAL,  //magic seen in early builds. Unused in final.
    DB_ARMOR, //Chest armor
    DB_SHIELD, 
    DB_WEAPON, 
    DB_CHEST, //loot pools for enemies and tresure
    DB_HELMET, //Head gear
    DB_CLOAK, //shoulder gear
    DB_GLOVE, //hand gear
    DB_RING,  //finger gear
    DB_WAND,  //rope and banner also use this slot
    DB_BELT,  //waist gear
    DB_BOOTS, //foot gear
    DB_POTION, //flasks and potions
    DB_SCROLL, //learn magic
    DB_KEYITEM, //Key item
    DB_AMULET,  //chest gear
    DB_DIALOUGEENTITY //Simpler entities for dialouge (signs, animals)
};
//set item ID by category and low byte
#define IDSet(name,n) (n|(DB_##name <<8))
#define IDMisc(n) IDSet(MISC,n) 
#define IDEnt(n) IDSet(ENTITY,n) 
#define IDSpell(n) IDSet(SPELL,n) 
#define IDRite(n) IDSet(RITUAL,n) 
#define IDArmor(n) IDSet(ARMOR,n) 
#define IDShield(n) IDSet(SHIELD,n) 
#define IDWeapon(n) IDSet(WEAPON,n) 
#define IDChest(n) IDSet(CHEST,n) 
#define IDHelm(n) IDSet(HELMET,n) 
#define IDCloak(n) IDSet(CLOAK,n) 
#define IDGlove(n) IDSet(GLOVE,n) 
#define IDRing(n) IDSet(RING,n) 
#define IDWand(n) IDSet(WAND,n) 
#define IDBelt(n) IDSet(BELT,n) 
#define IDBoot(n) IDSet(BOOTS,n) 
#define IDPotion(n) IDSet(POTION,n) 
#define IDScroll(n) IDSet(SCROLL,n) 
#define IDKeyItem(n) IDSet(KEYITEM,n) 
#define IDMedal(n) IDSet(AMULET,n) 
#define IDDEnt(n) IDSet(DIALOUGEENTITY,n) 


#define ITEMIDTYPE(x) (x >> 8) //get upper byte of the Item ID
#define IDInd(x) (x&0xFF) //get the lower byte of the Item ID

#define IDEntInd(n) IDEnt(entityList[EntInd_##n##]) //common macro for getting entity ID

enum ItemIDS{
    //misc items

    Item_TxominLetter=IDMisc(0),
    Item_Amaranth,
    Item_OrianaLetter,
    Item_KitarakLetter,
    Item_RabisatAsp,
    Item_Map1,
    Item_Map2, //reveals Erromon. Found in Gwernia stable
    
    //THERE IS NO MAP 3

    Item_Map4,
    Item_Map5,
    Item_Map6,
    Item_Map7,
    Item_Map8,
    Item_Map9,
    Item_Map10,
    Item_Sapphire,
    Item_Map11,
    Item_Sulphur, //unused item, can cause game to crash if loading description.
    Item_Spice, //component for spells and magic. found on Pepper plants
    Item_Herb, //component for spells and magic. found in clover patches
    Item_Gemstone, //component for spells and magic.
    Item_Map12,
    Item_Exp2, //unused
    Item_Exp3, //unused
    Item_Exp4, //unused
    Item_Exp5, //unused
    Item_CradawghBody,
    Item_Map13, //Should show region on pause map, but bugged not to. Cradawgh's Body does, instead.
    Item_Map14,
    Item_Map15,
    Item_Map16,
    Item_GoblinMap, //"Map to Goblin Lair". Unused
    Item_Map17,
    Item_Map18,
    Item_HellhoundHide,
    Item_DarkenbatHide,
    Item_BeastHide, //crafts Beast Hide armor
    Item_Chitlin,

    //Entites (using internal names as base)
    
    //NOTE: Shopkeeps/skill trainers use abbreviated names:
    //"ST*" designates them as shopkeep/Skill Trainer
    //the second pair of letters is the town from which they operate
    //after that is the identifiying numbers

    Ent_Charisa=IDEnt(0),
    Ent_Edur,
    Ent_DryadFemale,
    Ent_JundarMale,
    Ent_Male1,
    Ent_Male2,
    Ent_Guard1,
    Ent_Wyvern,
    Ent_GoblinScout2,
    Ent_Male3,
    Ent_DougalSuper,
    Ent_Yuli,
    Ent_ChaosLt,
    Ent_ChaosMjr,
    Ent_Female1,
    Ent_ChaosScout,
    Ent_ChaosWarrior,
    Ent_ChaosShocktrooper,
    Ent_ChaosMauler,
    Ent_Ehud, //aka Shamsuk
    Ent_Tamberlain,
    Ent_Goblin,
    Ent_GoblinSgt,
    Ent_Gorgon,
    Ent_Harpy,
    Ent_Cyclops,
    Ent_Hobgoblin,
    Ent_HumanBandit,
    Ent_BanditBoss,
    Ent_Kitarak,
    Ent_LizardMan,
    Ent_LizardManBoss,
    Ent_LizardManSgt,
    Ent_Marquis,
    Ent_Ogre,
    Ent_OgreBoss,
    Ent_Pochanargat,
    Ent_Rooughah,
    Ent_Shadow,
    Ent_Troll,
    Ent_Ardra,
    Ent_Bowden,
    Ent_GoblinPoisoner2,
    Ent_GoblinSgt2,
    Ent_Cradawgh,
    Ent_Female2,
    Ent_GoblinWarrior2,
    Ent_Female3,
    Ent_Lloyd, //aka Phelan
    Ent_MirariMale,
    Ent_Gabrion,
    Ent_Gechina,
    Ent_Gotzone,
    Ent_MirariFemale,
    Ent_Oriana,
    Ent_Pandara,
    Ent_Sheridan,
    Ent_Trahern,
    Ent_Txomin,
    Ent_Yeraza,
    Ent_Zaratas,
    Ent_Zurene,
    Ent_Abrecan,
    Ent_Alaron,
    Ent_Arturo,
    Ent_Baird,
    Ent_Becan,
    Ent_Brenna,
    Ent_Hobgoblin2,
    Ent_ChaosStormtrooper,
    Ent_Donovan,
    Ent_Dougal,
    Ent_Godric,
    Ent_Keelin,
    Ent_Niesen,
    Ent_ChaosSorcerer,
    Ent_Rheda,
    Ent_Sholeh,
    Ent_GoblinScout,
    Ent_HumanWoodsman,
    Ent_OgreGuard,
    Ent_Darkenbat,
    Ent_LargeScorpion,
    Ent_SkeletonArcher,
    Ent_SkeletonUnarmed,
    Ent_JundarGuard,
    Ent_Guard2,
    Ent_STUG16,
    Ent_DryadMale,
    Ent_GiantBoar,
    Ent_Woodsman3,
    Ent_Woodsman2,
    Ent_BanditBoss3,
    Ent_BanditBoss2,
    Ent_HumanBandit3,
    Ent_HumanBandit2,
    Ent_LavaHound,
    Ent_Firelord,
    Ent_Lugash,
    Ent_Skeleton,
    Ent_Wight,
    Ent_Wraith,
    Ent_Zombie,
    Ent_AirElem,
    Ent_Bear,
    Ent_Boar,
    Ent_DustDevil,
    Ent_EarthElem,
    Ent_FireElem,
    Ent_GiantBat,
    Ent_GiantRat,
    Ent_GiantScorpion,
    Ent_GiantSquid,
    Ent_Gryphon,
    Ent_HellHound,
    Ent_Manticore,
    Ent_Minotuar,
    Ent_Salamander,
    Ent_SandWorm,
    Ent_StoneGolem,
    Ent_WaterElem,
    Ent_Wolf,
    Ent_Dracovern,
    Ent_ChaosSlayer,
    Ent_ChaosSpellweaver,
    Ent_Guard3,
    Ent_JundarFemale,
    Ent_MinoLord,
    Ent_STAI01,
    Ent_STAI02,
    Ent_STAI03,
    Ent_STEU01,
    Ent_GoblinPoisoner,
    Ent_GiantSkeleton,
    Ent_STEU02,
    Ent_SpiritWolf,
    Ent_DireWolf,
    Ent_GiantGolem,
    Ent_TombRat,
    Ent_CaveBear,
    Ent_PlagueZombie,
    Ent_STER0903,
    Ent_STER0904,
    Ent_STER1002,
    Ent_STMI01,
    Ent_STMI02,
    Ent_Behrooz,
    Ent_Nasim,
    Ent_Golnar,
    Ent_Mehrdad,
    Ent_Shatrevar,
    Ent_Ksathra,
    Ent_Assim,
    Ent_Farris,
    Ent_STMI03,
    Ent_STMI04,
    Ent_STMI05,
    Ent_STPS01,
    Ent_STPS02,
    Ent_STPS03,
    Ent_STPS04,
    Ent_STPS05,
    Ent_STPS06,
    Ent_STPS07,
    Ent_STPS08,
    Ent_STPS09,
    Ent_STPS10,
    Ent_STTA01,
    Ent_STTA02,
    Ent_STTA03,
    Ent_STTA04,
    Ent_STTA05,
    Ent_STTA06,
    Ent_STTA07,
    Ent_STTA08,
    Ent_STTA09,
    Ent_STTA10,
    Ent_STTA11,
    Ent_STTA12,
    Ent_STTA13,
    Ent_STTE01,
    Ent_STTE02,
    Ent_STTE03,
    Ent_STTE04,
    Ent_STTE05,
    Ent_STTE06,
    Ent_STTE07,
    Ent_STTE08,
    Ent_STTE09,
    Ent_STTE10,
    Ent_STER1101,
    Ent_STER1202,
    Ent_STER1203,
    Ent_STER1301,
    Ent_STER1402,
    Ent_STUG01,
    Ent_STUG02,
    Ent_STUG03,
    Ent_NightBoss3,
    Ent_NightBoss2,
    Ent_NightBoss1,
    Ent_STUG17,
    Ent_STER1403,
    Ent_STER1404,
    Ent_STUG04,
    Ent_STUG05,
    Ent_STUG06,
    Ent_STUG07,
    Ent_STUG08,
    Ent_STUG09,
    Ent_STUG10,
    Ent_STUG11,
    Ent_STUG12,
    Ent_STUG13,
    Ent_STUG14,
    Ent_STUG15,
    Ent_STER1405,
    Ent_STER1504,
    Ent_STER1505,
    Ent_Xibid,
    Ent_UNK02DC, //only gap in the DB. unknown.
    Ent_Cadme,

    //Spells

    Spell_AirShield=IDSpell(0),
    Spell_ControlElem,
    Spell_Debilitation,
    Spell_DragonFlames,
    Spell_EarthSmite,
    Spell_Fireball,
    Spell_Lightning,
    Spell_Strength,
    Spell_Wind,
    Spell_Banishing,
    Spell_Charming,
    Spell_ControlMarquis,
    Spell_DetectingTraps,
    Spell_Endurance,
    Spell_Opening,
    Spell_SenseAura,
    Spell_Weakness,
    Spell_ControlZombies,
    Spell_CrushingDeath,
    Spell_Darkness,
    Spell_Haste,
    Spell_Exhaustion,
    Spell_Stamina,
    Spell_TapStamina,
    Spell_WallOfBones,
    Spell_SpiritShield,
    Spell_VsElemental,
    Spell_VsNaming,
    Spell_VsNecromancy,
    Spell_VsStar,
    Spell_DispelElemental,
    Spell_DispelNaming,
    Spell_DispelNecromancy,
    Spell_DispelStar,
    Spell_StarlightShield,
    Spell_DetectMoonPhase,
    Spell_DetectSunPhase,
    Spell_Dexterity,
    Spell_Clumsiness,
    Spell_FrozenDoom,
    Spell_Light,
    Spell_Stealth,
    Spell_StellarGravity,
    Spell_WebOfStarlight,
    Spell_WhiteFire,
    Spell_Poison,
    Spell_WraithTouch,
    Spell_AuraOfDeath,
    Spell_SolarWrath,
    Spell_Stupidity,
    Spell_Brilliance,
    Spell_RemovePoison,
    Spell_Photosynthesis,
    Spell_Teleportation,
    Spell_AcidBolt,
    Spell_Escape,
    Spell_Immolation,
    Spell_Mirror,
    Spell_CheatDeath,


    //Rituals (Omitted in final game, but listed for posterity.)
    
    Rite_Fog=IDRite(0), //Conjuring Fog
    Rite_Conjoining, // Conjoining
    Rite_Amulet,// Amulet Creation / Create Haste Amulet
    Rite_EnchantingShield,// Enchanting Shield
    Rite_RuneThrown,// Rune Thrown Weapon
    Rite_STGem,// Create ST Gem
    Rite_Hammer,// Rune Hammer
    Rite_Armor,// Enchanting Armor
    Rite_Sword,// Rune Sword
    Rite_Polearm,// Rune Pole Weapon
    Rite_Missle,// Rune Missile Weapon
    Rite_Wolf,// Summon Wolf
    Rite_Bear,// Summon Bear
    Rite_Elemental,// Summon Elemental
    Rite_ShieldAmulet,// Create Shield Amulet
    Rite_Rain,// Conjuring Rain
    Rite_Restoration,// Restoration

    //Armor

    Armor_Cloth=IDArmor(0),
    Armor_Leather,
    Armor_Scale,
    Armor_Chain,
    Armor_PartialPlate,
    Armor_FullPlate,
    Armor_ImprovedPlate,
    Armor_EnchHide,
    Armor_Dragon,
    Armor_JundarLeather,
    Armor_TalewokMail,
    Armor_TerminorMail,
    Armor_IdenScale,
    Armor_PomeScale,
    Armor_EnchPlate,
    Armor_RoyalPlate,
    Armor_Chaos,
    Armor_Scorpion=IDArmor(28),
    Armor_BeastHide=IDArmor(30),
    Armor_Hellhound,
    Armor_Darkenbat,
    Armor_exp1=IDArmor(36),
    Armor_exp2,
    Armor_exp3,
    Armor_exp4,
    Armor_exp5,
    Armor_exp6,
    Armor_exp7,
    Armor_exp8,
    Armor_exp9,
    Armor_exp10,
    Armor_exp11,
    Armor_exp12,
    Armor_exp13,
    Armor_exp14,
    Armor_exp15,
    Armor_exp16,
    Armor_exp17,
    Armor_exp18,
    Armor_exp19,
    Armor_exp20,
    Armor_exp21,
    Armor_exp22,
    Armor_exp23,
    Armor_exp24,
    Armor_unk3c,
    Armor_ChaosRobes,
    Armor_unk3e,
    Armor_Sheridan,
    Armor_unk40,
    Armor_Irondrake,

    //Shields
    
    Shield_Buckler=IDShield(17),
    Shield_Small,
    Shield_Large,
    Shield_Kite,
    Shield_Tower,
    Shield_Heater,
    Shield_Bronze,
    Shield_Jundar,
    Shield_Spirit,
    Shield_Hoplite,
    Shield_Chaos,
    Shield_Scorpion=IDShield(29),
    Shield_Sun=IDShield(33),
    Shield_Moon,
    Shield_Turtle,
    Shield_Dryad=IDShield(60),
    Shield_Sheridan=IDShield(62),
    Shield_Stardrake=IDShield(64),
    Shield_Wight=IDShield(66),
    Shield_Crab,

    //Weapons

    Weapon_BiteBear=IDWeapon(0),
    Weapon_BiteBuzzard,
    Weapon_BiteRat,
    Weapon_BiteWolf,
    Weapon_BreathPseudopod,
    Weapon_BreathDragon,
    Weapon_BreathGaze,
    Weapon_ClawAir,
    Weapon_ClawDragon,
    Weapon_ClawEarth,
    Weapon_ClawFire,
    Weapon_ClawTentacle,
    Weapon_ClawTroll,
    Weapon_ClawWraith,
    Weapon_ClawZombie,
    Weapon_BattleAxe,
    Weapon_BloodAxe,
    Weapon_Club,
    Weapon_Sythe,
    Weapon_GreatAxe,
    Weapon_Mace,
    Weapon_Maul,
    Weapon_MorningStar,
    Weapon_Warhammer,
    Weapon_GreatBow,
    Weapon_HunterBow,
    Weapon_LongBow,
    Weapon_Spikes,
    Weapon_ShortBow,
    Weapon_ThrowingIron,
    Weapon_Pike,
    Weapon_Poleaxe,
    Weapon_Spear,
    Weapon_Staff,
    Weapon_SpitVenom,
    Weapon_StingScorpion,
    Weapon_StingWyvern,
    Weapon_Broadsword,
    Weapon_Dagger,
    Weapon_ChaosSword,
    Weapon_Greatsword,
    Weapon_Longsword,
    Weapon_Gladius,
    Weapon_Sabre,
    Weapon_ShortSword,
    Weapon_Tanto,
    Weapon_ChaosTail,
    Weapon_Hatchet,
    Weapon_Javelin,
    Weapon_ChaosFlameblade,
    Weapon_BoarTusk,
    Weapon_MinotuarButt,
    Weapon_SwordLodin,
    Weapon_ArchmageStaff,
    Weapon_GiantAxe,
    Weapon_ElishevaSythe,
    Weapon_Warfang,
    Weapon_EhudStaff,
    Weapon_SwordTrahern,
    Weapon_LizardKingAxe,
    Weapon_IceStiletto,
    Weapon_ClawUnarmed,
    Weapon_LugashStaff,
    Weapon_JesterMace,
    Weapon_ChaosDeathwing,
    Weapon_exp4,
    Weapon_exp5,
    Weapon_exp6,
    Weapon_exp7,
    Weapon_exp8,
    Weapon_exp9,
    Weapon_exp10,
    Weapon_exp11,
    Weapon_exp12,
    Weapon_exp13,
    Weapon_exp14,
    Weapon_exp15,
    Weapon_CyclopsHurlstar,
    Weapon_CyclopsClub,
    Weapon_ChaosStaff,
    Weapon_StealthBlade,
    Weapon_ClawMarquis,
    Weapon_Sheridan,
    Weapon_ChaosSythe,
    Weapon_ChaosMaul,
    Weapon_HeartseekerBow,
    Weapon_EnchBlade,
    Weapon_FiredrakeFang,
    Weapon_StingPseudodragon,
    Weapon_SwordMight,
    Weapon_DragonFang,
    Weapon_BiteSpirit,
    Weapon_ClawPlague,
    Weapon_PoisonDart,
    Weapon_ThunderBow,
    Weapon_ShieldBow,
    Weapon_AccuracyBow,
    Weapon_GloryMace,
    Weapon_SpellbreakerAxe,
    Weapon_DartDistance,
    Weapon_BreklorFirestaff,
    Weapon_ThrowingKnife,
    Weapon_IronwoodStaff,
    Weapon_Lightreaver,
    Weapon_HockeyStick,

    //loot pools (internally referred as "chests")

    Loot_Poor=IDChest(0),
    Loot_TypicalWarrior,
    Loot_LowWizard,
    Loot_HighWizard,
    Loot_ChaosDude,
    Loot_DropAll,
    Loot_MiddleClass,
    Loot_FairWealth,
    Loot_RichDude,
    Loot_HealthPak,
    Loot_Ehud,
    Loot_Kitarak,
    Loot_LizardBoss, //also used in enemy randomizer "failsafe"
    Loot_Lugosh,
    Loot_OgreBoss,
    Loot_Pandara,
    Loot_Sheridan,
    Loot_BanditBoss,
    Loot_Barrowwight_2,
    Loot_Barrowwight_6,
    Loot_Barrowwight_5,
    Loot_Barrowwight_1,
    Loot_Barrowwight_3,
    Loot_Barrowwight_7,
    Loot_Barrowwight_4,
    Loot_Marquis,
    Loot_GoblinWarrior,
    Loot_Dryad,
    Loot_Scorpion,
    Loot_Beast,
    Loot_Hellhound,
    Loot_Darkenbat,
    Loot_ChaosLt,
    Loot_ChaosMjr,
    Loot_ChaosWizard,
    Loot_Gorgon,
    Loot_Harpy,
    Loot_Cyclops,
    Loot_Lizardman,
    Loot_Minotuar,
    Loot_Ogre,
    Loot_Troll,
    Loot_Wight,
    Loot_Skeleton,
    Loot_GenChest,
    Loot_Manticore,
    Loot_CradawghPotions,
    Loot_CradawghScrolls1,
    Loot_CradawghScrolls3,
    Loot_G10_1,
    Loot_GoblinStronghold1,
    Loot_GoblinStronghold4,
    Loot_G08_2,
    Loot_G09_1,
    Loot_OgreCaveArtifact,
    Loot_BitsOUsefulStuff,
    Loot_H08_1,
    Loot_MinorHealthpack,
    Loot_MediumHealthpack,
    Loot_C07Tent1,
    Loot_PoorGoblin,
    Loot_GoodGoblin,
    Loot_L08_1,

    //headgear

    Helm_Kendal=IDHelm(0),
    Helm_Tempests,
    Helm_Defence,
    Helm_Wisdom,
    Helm_Charisma,
    Helm_Wizard=IDHelm(6),
    Helm_SpiritDrake=IDHelm(38),

    //cloaks

    Cloak_Leather=IDCloak(0),
    Cloak_Phantom,
    Cloak_Mirari,
    Cloak_Nightdrake=IDCloak(40),

    //gloves

    Glove_Healing=IDGlove(0),
    Glove_Tinker,
    Glove_Bard,
    Glove_Jundar,
    Glove_Plate,
    Glove_Stormdrake=IDGlove(41),

    //Rings

    Ring_Lunar=IDRing(8),
    Ring_Witch,
    Ring_Ether,
    Ring_Healing=IDRing(31),
    Ring_Magedrake=IDRing(39),
    Ring_Namer=IDRing(49),

    //Wands (and stuff that goes in the "Wands" slot)

    Wand_Banner=IDWand(0),
    Wand_MoonGem,
    Wand_Stormbreaker,
    Wand_HornKynon,
    Wand_HarpIgnore,
    Wand_VsStar,
    Wand_VsNecro,
    Wand_VsNaming,
    Wand_VsElement,
    Wand_TapStamina,
    Wand_Fireball,
    Wand_11, // unknown, omitted even in earlest available build. still used in several lists
    Wand_Gravity,
    Wand_Persuasion,
    Wand_Shielding,
    Wand_Light,
    Wand_Darkness,
    Wand_FrozenDoom,
    Wand_CrushingDeath,
    Wand_WebOfStarlight,
    Wand_Starfire,
    Wand_WraithTouch,
    Wand_Lightning,
    Wand_Acid,
    Wand_Immolation,
    Wand_Banishing,
    Wand_Revival,
    Wand_GemSensing,
    Wand_GemAspect,
    Wand_Rope,
    Wand_WallOfBones=IDWand(124),

    //belts

    Belt_Life=IDBelt(0),
    Belt_Teleport,
    Belt_Reflect,
    Belt_Mercenary=IDBelt(7),

    //boots

    Boot_Woodsman=IDBoot(0),
    Boot_Adamant,
    Boot_Leather=IDBoot(4),
    Boot_Striding,
    Boot_Speed=IDBoot(42),

    //Potions
    //(NOTE: mind the captialization that differentiates it from the other potion enum)

    Potion_Fire=IDPotion(0),
    Potion_Inferno,
    Potion_Sleep,
    Potion_Acid,
    Potion_Healing,
    Potion_Stamina,
    Potion_Curing,
    Potion_Antidote,
    Potion_Restore,
    Potion_Strength,
    Potion_Dexterity,
    Potion_Aspect, //unused, code changes into Healing potion if found, renamed "A"
    Potion_Jumping, //unused, renamed "J"
    Potion_Clarity,
    Potion_Charisma,
    Potion_Defence,
    Potion_Steath,

    //Scrolls

    Scroll_SenseAura=IDScroll(0),
    Scroll_Oriana,
    Scroll_Wind,
    Scroll_Endurance,
    Scroll_Weakness,
    Scroll_Teleport,
    Scroll_6,// unknown, omitted even in earlest available build. still used in several lists
    Scroll_Strength,
    Scroll_RemovePoison,
    Scroll_Lightning,
    Scroll_Immolation,
    Scroll_11,// unknown, omitted even in earlest available build. still used in several lists
    Scroll_Escape,
    Scroll_EarthSmite,
    Scroll_DragonFlames,
    Scroll_Debilitation,
    Scroll_ControlElem,
    Scroll_AirShield,
    
    //scrolls 17-45 appear to be ommitted
    //from the earliest available build.
    //May have been the ritual scrolls?
    
    Scroll_Lighthouse=IDScroll(46),
    Scroll_47, //Unknown
    Scroll_48, //Unknown
    Scroll_49, //Unknown
    Scroll_Fireball,
    Scroll_AcidBolt,
    Scroll_AuraOfDeath,
    Scroll_SolarWrath,
    Scroll_Banishing,
    Scroll_Brilliance,
    Scroll_Charming,
    Scroll_CheatDeath,
    Scroll_Command,
    Scroll_ControlMarquis,
    Scroll_ControlZombie,
    Scroll_CrushingDeath,
    Scroll_Darkness,
    Scroll_DetectMoonPhase,
    Scroll_DetectSunPhase,
    Scroll_DetectChaos,
    Scroll_DetectTraps,
    Scroll_Dexterity,
    Scroll_DispelElemental,
    Scroll_DispelNaming,
    Scroll_DispelNecro,
    Scroll_DispelStar,
    Scroll_Clumsiness,
    Scroll_FrozenDoom,
    Scroll_Haste,
    Scroll_KnowAspect,
    Scroll_Light,
    Scroll_Mirror,
    Scroll_Stupidity,
    Scroll_Opening,
    Scroll_Photosynth,
    Scroll_ShieldStarlight,
    Scroll_Exhaustion,
    Scroll_ShieldSpirit,
    Scroll_Stamina,
    Scroll_Stealth,
    Scroll_StellarGrav,
    Scroll_TapStamina,
    Scroll_Teleportation,
    Scroll_VsElemental,
    Scroll_VsNaming,
    Scroll_VsNecro,
    Scroll_VsStar,
    Scroll_WallOfBones,
    Scroll_WebOfStarlight,
    Scroll_Whitefire,


    //Key Items

    Key_Bowden=IDKeyItem(0), //Unlocks door granting Trahern's Sword
    Key_Black,
    Key_Skull,//unused, unique sprite
    Key_Blood,//unused, unique sprite
    Key_Bone,//unused, unique sprite
    Key_Lighthouse,
    Key_lodin,
    Key_Dragon,
    Key_17,//unused
    Key_16,//unused
    Key_15,//unused
    Key_14,//unused
    Key_13,//unused
    Key_12,//unused
    Key_11,//unused
    Key_10,//unused
    Key_9,//unused
    Key_8,//unused
    Key_7,//unused
    Key_6,//unused
    Key_5,//unused
    Key_4,//unused
    Key_3,//unused
    Key_2,//unused
    Key_1,//unused

    //Amulets
    Amulet_Pandara=IDMedal(0),
    Amulet_Elisheva,
    Amulet_Shamsuk,
    Amulet_STGem,//Unused. Would originally be made with Ritual magic
    Amulet_Haste,
    Amulet_Shield,
    Amulet_Mirror,
    Amulet_Pork,
    Amulet_08, //Unknown
    Amulet_Marquis,

    //Dialouge Entities

    DEnt_Amann=IDDEnt(0),
    DEnt_Balfin,
    DEnt_Swagma,
    DEnt_Pillar,//unlisted in appearance getters
    DEnt_Cadme,
    DEnt_Cedric,
    DEnt_Pochanargat,//unlisted in appearance getters
    DEnt_Damek,
    DEnt_Darmath,
    DEnt_Devlin,
    DEnt_10, //Unknown
    DEnt_Esmerelda,
    DEnt_12,//Unknown
    DEnt_Fyrsil,
    DEnt_Gavinn,
    DEnt_Herne,
    DEnt_Honza,
    DEnt_17,//unknown.
    DEnt_Lycea,
    DEnt_Mago,
    DEnt_Meriona,
    DEnt_21,//Unknown.
    DEnt_Niala,
    DEnt_23, //unknown.
    DEnt_Tamberlain,//unlisted in appearance getters
    DEnt_Xibid,
    DEnt_Batrin,
    DEnt_Cow,  //shown as a horse, for some reason.
    DEnt_Chicken,
    DEnt_Horse,
    DEnt_Nurse,
    DEnt_Kolokos,
    DEnt_Gillek,
    DEnt_Jundargate,
    DEnt_Baldric,
    DEnt_Bodecia,//unlisted in appearance getters
    DEnt_KingdomSign,
    DEnt_MirariSign,
    DEnt_TerminorSign,
    DEnt_JundarSign,
    DEnt_Exponto,
    DEnt_Fish, //shown as a horse, for some reason.
    DEnt_Flock,//shown as a horse, for some reason.

    Item_NONE=-1 //item was not parsed correctly or desn't exist
};

//indecies of their order in the ROM databases. Used in GetIDIndex()

enum ItemInd{
    ItemInd_Cradawgh,
    ItemInd_Map18,
    ItemInd_Map17,
    ItemInd_Map16,
    ItemInd_Map15,
    ItemInd_Map14,
    ItemInd_Map13,
    ItemInd_Map12,
    ItemInd_Map11,
    ItemInd_Map10,
    ItemInd_Map9,
    ItemInd_Map8,
    ItemInd_Map7,
    ItemInd_Map6,
    ItemInd_Map5,
    ItemInd_Map4,
    ItemInd_Map2,
    ItemInd_Map1,
    ItemInd_Asp,
    ItemInd_LetterKitarak,
    ItemInd_LetterOriana,
    ItemInd_Amaranth,
    ItemInd_LetterTxomin,
    ItemInd_MapGoblin,
    ItemInd_exp5,
    ItemInd_exp4,
    ItemInd_exp3,
    ItemInd_exp2,
    ItemInd_Sapphire,
    ItemInd_Sulphur,
    ItemInd_Spice,
    ItemInd_Herb,
    ItemInd_Gemstone,
    ItemInd_HideHellhound,
    ItemInd_HideDarkenbat,
    ItemInd_HideBeast,
    ItemInd_HideChitlin,
    
    ItemInd_HelmCharisma,
    ItemInd_HelmWisdom,
    ItemInd_HelmDefence,
    ItemInd_HelmTempest,
    ItemInd_HelmKendal,
    ItemInd_HelmSpiritdrake,
    ItemInd_HelmWizard,

    ItemInd_CloakNightdrake,
    ItemInd_CloakLeather,
    ItemInd_CloakPhantom,
    ItemInd_CloakMirari,

    ItemInd_GlovePlate,
    ItemInd_GloveBard,
    ItemInd_GloveTinker,
    ItemInd_GloveHealing,
    ItemInd_GloveStormdrake,
    ItemInd_GloveJundar,
    
    ItemInd_RingHealing,
    ItemInd_RingMagedrake,
    ItemInd_RingNamer,
    ItemInd_RingLunar,
    ItemInd_RingWitch,
    ItemInd_RingEther,

    ItemInd_Rope,
    ItemInd_GemAspect,
    ItemInd_GemSensing,

    ItemInd_WandRevival,
    ItemInd_WandBanishing,
    ItemInd_WandImmolation,
    ItemInd_WandAcid,
    ItemInd_WandLighning,
    ItemInd_WandWraithTouch,
    ItemInd_WandStarfire,
    ItemInd_WandWebOfStarlight,
    ItemInd_WandCrushingDeath,
    ItemInd_WandFrozenDoom,
    ItemInd_WandDarkness,
    ItemInd_WandLight,
    ItemInd_WandShielding,
    ItemInd_WandPersuasion,
    ItemInd_WandGravity,
    ItemInd_WandFireball,
    ItemInd_WandTapStamina,
    ItemInd_WandVsElement,
    ItemInd_WandVsNaming,
    ItemInd_WandVsNecro,
    ItemInd_WandVsStar,
    ItemInd_WandWallOfBones,
    
    ItemInd_Banner,
    ItemInd_MoonGem,
    ItemInd_Stormbreaker,
    ItemInd_Wand11,

    ItemInd_HornKynon,
    ItemInd_HarpIgnore,

    ItemInd_AmuletHaste=192,
};

enum EntInd{
    EntInd_STER1505,
    EntInd_STER1504,
    EntInd_STER1405,
    EntInd_STER1404,
    EntInd_STER1403,
    EntInd_STER1402,
    EntInd_STER1301,
    EntInd_STER1203,
    EntInd_STER1202,
    EntInd_STER1101,
    EntInd_STER1002,
    EntInd_STER0904,
    EntInd_STER0903,
    EntInd_STUG17,
    EntInd_STUG16,
    EntInd_STUG15,
    EntInd_STUG14,
    EntInd_STUG13,
    EntInd_STUG12,
    EntInd_STUG11,
    EntInd_STUG10,
    EntInd_STUG09,
    EntInd_STUG08,
    EntInd_STUG07,
    EntInd_STUG06,
    EntInd_STUG05,
    EntInd_STUG04,
    EntInd_STUG03,
    EntInd_STUG02,
    EntInd_STUG01,
    EntInd_STTE10,
    EntInd_STTE09,
    EntInd_STTE08,
    EntInd_STTE07,
    EntInd_STTE06,
    EntInd_STTE05,
    EntInd_STTE04,
    EntInd_STTE03,
    EntInd_STTE02,
    EntInd_STTE01,
    EntInd_STTA13,
    EntInd_STTA12,
    EntInd_STTA11,
    EntInd_STTA10,
    EntInd_STTA09,
    EntInd_STTA08,
    EntInd_STTA07,
    EntInd_STTA06,
    EntInd_STTA05,
    EntInd_STTA04,
    EntInd_STTA03,
    EntInd_STTA02,
    EntInd_STTA01,
    EntInd_STPS10,
    EntInd_STPS09,
    EntInd_STPS08,
    EntInd_STPS07,
    EntInd_STPS06,
    EntInd_STPS05,
    EntInd_STPS04,
    EntInd_STPS03,
    EntInd_STPS02,
    EntInd_STPS01,
    EntInd_STMI05,
    EntInd_STMI04,
    EntInd_STMI03,
    EntInd_STMI02,
    EntInd_STMI01,
    EntInd_STEU02,
    EntInd_STEU01,
    EntInd_STAI02,
    EntInd_STAI01,
    EntInd_Jundarguard,
    EntInd_MirariFemale,
    EntInd_MirariMale,
    EntInd_Female3,
    EntInd_Female2,
    EntInd_Female1,
    EntInd_Male3,
    EntInd_Male2,
    EntInd_Male1,
    EntInd_JundarFemale,
    EntInd_Guard3,
    EntInd_Guard2,
    EntInd_JundarMale,
    EntInd_Guard1,
    EntInd_STAI03,
    EntInd_NightBoss3,
    EntInd_NightBoss2,
    EntInd_NightBoss1,
    EntInd_DryadMale,
    EntInd_DryadFemale,
    EntInd_Woodsman3,
    EntInd_Woodsman2,
    EntInd_BanditBoss3,
    EntInd_BanditBoss2,
    EntInd_HumanBandit3,
    EntInd_HumanBandit2,
    EntInd_ChaosSlayer,
    EntInd_ChaosSpellweaver,
    EntInd_ChaosStormtrooper,
    EntInd_Hobgoblin2,
    EntInd_GoblinWarrior2,
    EntInd_GoblinSgt2,
    EntInd_GoblinPoisoner2,
    EntInd_GoblinScout2,
    EntInd_GoblinPoisoner,
    EntInd_OgreGuard,
    EntInd_HumanWoodsman,
    EntInd_GoblinScout,
    EntInd_ChaosSorceror,
    EntInd_ChaosScout,
    EntInd_ChaosWarrior,
    EntInd_ChaosShocktrooper,
    EntInd_ChaosMauler,
    EntInd_Goblin,
    EntInd_GoblinSgt,
    EntInd_Gorgon,
    EntInd_Harpy,
    EntInd_Cyclops,
    EntInd_Hobgoblin,
    EntInd_HumanBandit,
    EntInd_BanditBoss,
    EntInd_LizardMan,
    EntInd_LizardManSgt,
    EntInd_Ogre,
    EntInd_OgreBoss,
    EntInd_Troll,
    EntInd_Minotuar,
    EntInd_DougalSuper,
    EntInd_Cadme,
    EntInd_Yuli,
    EntInd_Xibid,
    EntInd_Tamberlain,
    EntInd_Rooughah,
    EntInd_Charisa,
    EntInd_Edur,
    EntInd_Ardra,
    EntInd_Bowden,
    EntInd_Cradawgh,
    EntInd_Lloyd,  //aka Phelan
    EntInd_Gabrion,
    EntInd_Gechina,
    EntInd_Gotzone,
    EntInd_Oriana,
    EntInd_Pandara,
    EntInd_Trahern,
    EntInd_Txomin,
    EntInd_Yeraza,
    EntInd_Zaratas,
    EntInd_Zurene,
    EntInd_Farris,
    EntInd_Abrecan,
    EntInd_Alaron,
    EntInd_Arturo,
    EntInd_Baird,
    EntInd_Becan,
    EntInd_Brenna,
    EntInd_Donovan,
    EntInd_Dougal,
    EntInd_Godric,
    EntInd_Keelin,
    EntInd_Niesen,
    EntInd_Rheda,
    EntInd_Sholeh,
    EntInd_MinotuarLord,
    EntInd_ChaosLt,
    EntInd_ChaosMjr,
    EntInd_Kitarak,
    EntInd_LizardManBoss,
    EntInd_Marquis,
    EntInd_Pochanargat,
    EntInd_Shadow,
    EntInd_Ehud,
    EntInd_Sheridan,
    EntInd_Ksathra,
    EntInd_Shatrevar,
    EntInd_Mehrdad,
    EntInd_Golnar,
    EntInd_Nasim,
    EntInd_Behrooz,
    EntInd_Assim,
    EntInd_PlagueZombie,
    EntInd_GiantSkeleton,
    EntInd_SkeletonUnarmed,
    EntInd_SkeletonArcher,
    EntInd_Lugash,
    EntInd_Skeleton,
    EntInd_Wight,
    EntInd_Wraith,
    EntInd_Zombie,
    EntInd_Firelord,
    EntInd_SpiritWolf,
    EntInd_GiantGolem,
    EntInd_AirElemental,
    EntInd_DustDevil,
    EntInd_EarthElemental,
    EntInd_FireElemental,
    EntInd_StoneGolem,
    EntInd_WaterElemental,
    EntInd_GiantBoar,
    EntInd_LavaHound,
    EntInd_Wyvern,
    EntInd_DireWolf,
    EntInd_TombRat,
    EntInd_CaveBear,
    EntInd_LargeScorpion,
    EntInd_Darkenbat,
    EntInd_Bear,
    EntInd_Boar,
    EntInd_GiantBat,
    EntInd_GiantRat,
    EntInd_GiantScorpion,
    EntInd_GiantSquid,
    EntInd_Gryphon,
    EntInd_HellHound,
    EntInd_Manticore,
    EntInd_Salamander,
    EntInd_SandWorm,
    EntInd_Wolf,
    EntInd_Dracovern,
};
//includes chest armor and shields
enum ArmorInd{
    ArmorInd_ChaosRobes,
    ArmorInd_Sheridan,
    ArmorInd_Irondrake,
    ArmorInd_exp24,
    ArmorInd_exp23,
    ArmorInd_exp22,
    ArmorInd_exp21,
    ArmorInd_exp20,
    ArmorInd_exp19,
    ArmorInd_exp18,
    ArmorInd_exp17,
    ArmorInd_exp16,
    ArmorInd_exp15,
    ArmorInd_exp14,
    ArmorInd_exp13,
    ArmorInd_exp12,
    ArmorInd_exp11,
    ArmorInd_exp10,
    ArmorInd_exp9,
    ArmorInd_exp8,
    ArmorInd_exp7,
    ArmorInd_exp6,
    ArmorInd_exp5,
    ArmorInd_exp4,
    ArmorInd_exp3,
    ArmorInd_exp2,
    ArmorInd_exp1,
    ArmorInd_Darkenbat,
    ArmorInd_Hellhound,
    ArmorInd_BeastHide,
    ArmorInd_Scorpion,
    ArmorInd_Cloth,
    ArmorInd_Leather,
    ArmorInd_Scale,
    ArmorInd_Chain,
    ArmorInd_PartialPlate,
    ArmorInd_FullPlate,
    ArmorInd_ImprovedPlate,
    ArmorInd_EnchHide,
    ArmorInd_Dragon,
    ArmorInd_JundarLeather,
    ArmorInd_TalewokMail,
    ArmorInd_TerminorMail,
    ArmorInd_IdenScale,
    ArmorInd_PomeScale,
    ArmorInd_EnchPlate,
    ArmorInd_RoyalPlate,
    ArmorInd_Chaos,
    ArmorInd_ShieldCrab,
    ArmorInd_ShieldWight,
    ArmorInd_ShieldDryad,
    ArmorInd_ShieldSheridan,
    ArmorInd_ShieldStardrake,
    ArmorInd_ShieldTurtle,
    ArmorInd_ShieldMoon,
    ArmorInd_ShieldSun,
    ArmorInd_ShieldScorpion,
    ArmorInd_ShieldBuckler,
    ArmorInd_ShieldSmall,
    ArmorInd_ShieldLarge,
    ArmorInd_ShieldKite,
    ArmorInd_ShieldTower,
    ArmorInd_ShieldHeater,
    ArmorInd_ShieldBronze,
    ArmorInd_ShieldJundar,
    ArmorInd_ShieldSpirit,
    ArmorInd_ShieldHoplite,
    ArmorInd_ShieldChaos,
};

enum WeaponInd{
    WeaponInd_BiteSpirit,
    WeaponInd_exp15,
    WeaponInd_exp14,
    WeaponInd_exp13,
    WeaponInd_exp12,
    WeaponInd_exp11,
    WeaponInd_exp10,
    WeaponInd_exp9,
    WeaponInd_exp8,
    WeaponInd_exp7,
    WeaponInd_exp6,
    WeaponInd_exp5,
    WeaponInd_exp4,
    WeaponInd_BiteBear,
    WeaponInd_BiteBuzzard,
    WeaponInd_BiteRat,
    WeaponInd_BiteWolf,
    WeaponInd_BreathPseudopod,
    WeaponInd_BreathDragon,
    WeaponInd_BreathGaze,
    WeaponInd_ClawMarquis,
    WeaponInd_ClawPlague,
    WeaponInd_ClawUnarmed,
    WeaponInd_ClawAir,
    WeaponInd_ClawDragon,
    WeaponInd_ClawEarth,
    WeaponInd_ClawFire,
    WeaponInd_ClawTentacle,
    WeaponInd_ClawTroll,
    WeaponInd_ClawWraith,
    WeaponInd_ClawZombie,
    WeaponInd_HockeyStick,
    WeaponInd_CyclopsClub,
    WeaponInd_SpellbreakerAxe,
    WeaponInd_GloryMace,
    WeaponInd_JesterMace,
    WeaponInd_ChaosFlameblade,
    WeaponInd_ChaosMaul,
    WeaponInd_LizardKingAxe,
    WeaponInd_ElishevaSythe,
    WeaponInd_GiantAxe,
    WeaponInd_BattleAxe,
    WeaponInd_BloodAxe,
    WeaponInd_Club,
    WeaponInd_Sythe,
    WeaponInd_GreatAxe,
    WeaponInd_Mace,
    WeaponInd_Maul,
    WeaponInd_MorningStar,
    WeaponInd_Warhammer,
    WeaponInd_ChaosSythe,
    WeaponInd_ChaosDeathwing,
    WeaponInd_Poleaxe,
    WeaponInd_AccuracyBow,
    WeaponInd_ShieldBow,
    WeaponInd_ThunderBow,
    WeaponInd_HeartseekerBow,
    WeaponInd_GreatBow,
    WeaponInd_HunterBow,
    WeaponInd_LongBow,
    WeaponInd_ShortBow,
    WeaponInd_IronwoodStaff,
    WeaponInd_ChaosStaff,
    WeaponInd_BreklorFirestaff,
    WeaponInd_LugashStaff,
    WeaponInd_EhudStaff,
    WeaponInd_ArchmageStaff,
    WeaponInd_Pike,
    WeaponInd_Spear,
    WeaponInd_Staff,
    WeaponInd_SpitVenom,
    WeaponInd_ChaosTail,
    WeaponInd_StingPseudodragon,
    WeaponInd_StingScorpion,
    WeaponInd_StingWyvern,
    WeaponInd_Lightreaver,
    WeaponInd_SwordMight,
    WeaponInd_StealthBlade,
    WeaponInd_Sheridan,
    WeaponInd_EnchBlade,
    WeaponInd_FiredrakeFang,
    WeaponInd_IceStiletto,
    WeaponInd_SwordTrahern,
    WeaponInd_Warfang,
    WeaponInd_SwordLodin,
    WeaponInd_Broadsword,
    WeaponInd_Dagger,
    WeaponInd_ChaosSword,
    WeaponInd_Greatsword,
    WeaponInd_Longsword,
    WeaponInd_Gladius,
    WeaponInd_Sabre,
    WeaponInd_ShortSword,
    WeaponInd_Tanto,
    WeaponInd_DragonFang,
    WeaponInd_ThrowingIron,
    WeaponInd_ThrowingKnife,
    WeaponInd_CyclopsHurlstar,
    WeaponInd_DartDistance,
    WeaponInd_PoisonDart,
    WeaponInd_Spikes,
    WeaponInd_Hatchet,
    WeaponInd_Javelin,
    WeaponInd_BoarTusk,
    WeaponInd_MinotuarButt,
};

enum LootInd{
    LootInd_L08_1,
    LootInd_GoodGoblin,
    LootInd_PoorGoblin,
    LootInd_C07Tent1,
    LootInd_MediumHealthpack,
    LootInd_MinorHealthpack,
    LootInd_H08_1,
    LootInd_BitsOUsefulStuff,
    LootInd_OgreCaveArtifact,
    LootInd_G09_1,
    LootInd_G08_2,
    LootInd_GoblinStronghold4,
    LootInd_GoblinStronghold1,
    LootInd_G10_1,
    LootInd_CradawghScrolls3,
    LootInd_CradawghScrolls1,
    LootInd_CradawghPotions,
    LootInd_Manticore,
    LootInd_GenChest,
    LootInd_Skeleton,
    LootInd_Wight,
    LootInd_Troll,
    LootInd_Ogre,
    LootInd_Minotuar,
    LootInd_Lizardman,
    LootInd_Cyclops,
    LootInd_Harpy,
    LootInd_Gorgon,
    LootInd_ChaosWizard,
    LootInd_ChaosMjr,
    LootInd_ChaosLt,
    LootInd_Darkenbat,
    LootInd_Hellhound,
    LootInd_Beast,
    LootInd_Scorpion,
    LootInd_Dryad,
    LootInd_GoblinWarrior,
    LootInd_Marquis,
    LootInd_Barrowwight_4,
    LootInd_Barrowwight_7,
    LootInd_Barrowwight_3,
    LootInd_Barrowwight_1,
    LootInd_Barrowwight_5,
    LootInd_Barrowwight_6,
    LootInd_Barrowwight_2,
    LootInd_BanditBoss,
    LootInd_Sheridan,
    LootInd_Pandara,
    LootInd_OgreBoss,
    LootInd_Lugosh,
    LootInd_LizardBoss,
    LootInd_Kitarak,
    LootInd_Ehud,
    LootInd_HealthPak,
    LootInd_RichDude,
    LootInd_FairWealth,
    LootInd_MiddleClass,
    LootInd_DropAll,
    LootInd_ChaosDude,
    LootInd_HighWizard,
    LootInd_LowWizard,
    LootInd_TypicalWarrior,
    LootInd_Poor,
};

enum DiaEntInd{
    DiaEntInd_Pochanargat,//unlisted in appearance getters
    DiaEntInd_Pillar,//unlisted in appearance getters
    DiaEntInd_Flock,//shown as a horse, for some reason.
    DiaEntInd_Fish,//shown as a horse, for some reason.
    DiaEntInd_Exponto,
    DiaEntInd_JundarSign,
    DiaEntInd_TerminorSign,
    DiaEntInd_MirariSign,
    DiaEntInd_KingdomSign,
    DiaEntInd_Bodecia,//unlisted in appearance getters
    DiaEntInd_Baldric,
    DiaEntInd_Jundargate,
    DiaEntInd_Gillek,
    DiaEntInd_Swagma,
    DiaEntInd_Kolokos,
    DiaEntInd_Horse,
    DiaEntInd_Chicken,
    DiaEntInd_Cow,//shown as a horse, for some reason.
    DiaEntInd_Batrin,
    DiaEntInd_Xibid,
    DiaEntInd_Tamberlain,//unlisted in appearance getters
    DiaEntInd_Niala,
    DiaEntInd_Meriona,
    DiaEntInd_Mago,
    DiaEntInd_Lycea,
    DiaEntInd_Herne,
    DiaEntInd_Honza,
    DiaEntInd_Gavinn,
    DiaEntInd_Fyrsil,
    DiaEntInd_Esmerelda,
    DiaEntInd_Devlin,
    DiaEntInd_Darmath,
    DiaEntInd_Damek,
    DiaEntInd_Cedric,
    DiaEntInd_Cadme,
    DiaEntInd_Balfin,
    DiaEntInd_Amann,
};