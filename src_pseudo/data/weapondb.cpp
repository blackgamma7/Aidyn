...
void WeaponDB::Orphaned() {
  ...
  u8 bVar5;
  u32 aiStack_30 = 0;
  load_db_array_size(&weaponDBROM,&this->Total,&aiStack_30);
  bVar5 = 0;
  ALLOCS(this->weapons,this->Total*sizeof(WeaponRam),0x34);
  for(u8 i=0;i<11;i++) {
    load_db_array_size(&weaponDBROM,this->Types + i,&aiStack_30);
...
void WeaponDB::Load(u8 index,int *pos) {
  ...
  ROMCOPYS(&fromROM,weaponDBROM + *pos,sizeof(Weapon_ROM),78);
...
void WeaponDB::Init() {
  ...
  u8 index;
  u32 auStack_30 = 0;
  load_db_array_size(&weaponDBROM,&this->Total,&auStack_30);
  index = 0;
  uVar3 = 0;
  ALLOCS(this->weapons,this->Total*sizeof(WeaponRam),378);
  for(u8 i=0;i<11;i++) {
    load_db_array_size(&weaponDBROM,this->Types + i,&auStack_30);
