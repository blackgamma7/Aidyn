...
s32 Party::DiplomatCheck(){
  ...
  uVar4 = RollD(1,100);
  if (uVar4 < uVar3)
    return ::gSkillCheck(uVar3 - uVar4);
  else return 0;
}
...
s32 Party::LoremasterCheck(){
  ...
  if (rand < total) return ::gSkillCheck((total - rand));
  else return 0;
}
...
u32 Party::UnusedLoremasterCheck(){
  ...
  if (cVar4 != -1) {
    ...
    uVar3 = RollD(1,100);
    if (uVar3 < uVar5) return ::gSkillCheck((uVar5 - uVar3));
  }
  return 0;
}
...
u32 Party::UnusedRangerCheck(){
  ...
  if (uVar3 != 4) {
    ...
    uVar3 = RollD(1,100);
    if (uVar3 < uVar7) {
      return ::gSkillCheck((uVar7 - uVar3));
    }
  }
  return 0;
}
...
u8 Party::GetMemberWarriorIntStam(u8 param_2){
  ...
  if (pCVar1) {
    ...
    uVar4 = RollD(1,100);
    if (uVar4 < uVar7) uVar6 = ::gSkillCheck((uVar7 - uVar4));
    else uVar6 = 0;
  }
  return uVar6;
}
    ...
    if (this->Inventory->TakeItem(itemID_array[ItemInd_Herb],1)) {
      ...
      if ((s16)skillMod <= CharStats::getModded(ent->Stats,STAT_STAM)) {
        ...
        if ((s16)uVar8 < skillMod) {
          Entity::addHP(pCVar1,::gSkillCheck((skillMod - (s16)uVar8)));
...
u32 Party::CraftPotion(u8 user,u8 item){
  ...
  if (recepie) {
    ...
    if (roll < vsRoll) {
      ...
      if (recepie->unk2 <= ::gSkillCheck(vsRoll - roll)) {
...
u32 Party::MechanicCheck(){
  ...
  if (cVar4 == -1) return 0;
    ...
    return ::gSkillCheck(((RollD(1,100) - (cVar4 * 5 + iVar2 + iVar3 & 0x7fffU) * -2)));
  }
}
...
u32 Party::MerchantCheck(){
  ...
  return ::gSkillCheck((RollD(1,100) - (uVar3 * 3 + i * 10)));
}
...
u32 Party::RangerCheck(){
  ...
  return  ::gSkillCheck(RollD(1,100) - (r * 10 + 0x50));
}
...
u32 Party::ThiefCheck(){
  ...
  if (cVar4 == -1) return 0;
    ...
    return ::gSkillCheck(RollD(1,100) - (iVar2 * 4 + cVar4 * 10));
  }
}
